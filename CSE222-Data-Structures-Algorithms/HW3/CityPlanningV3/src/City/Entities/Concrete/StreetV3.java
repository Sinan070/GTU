...
public class StreetV3 extends StreetVersion {

    private LDLinkedList<Building> _buildings;


    public StreetV3(int lenght){
        super(lenght);
        _buildings = new LDLinkedList<>();
    ...
    public void addBuilding(Building building) {
        if(checkPosition(building)){
            ...
            ++_buildingNumber;
            if(building.getSide().equals("r") ){ ++_rightSideNum;}
            else if(building.getSide().equals("l") ){++_lefSideNum;}
        }
        else System.out.println("There is no enough space to locate the " + building.getClass().getSimpleName());
    }

    /**
     * Remove a building from the list of buildings
     *
     * @param Id The id of the building to be deleted.
     */
    @Override
    public void deleteBuilding(int Id) throws NoSuchElementException {
        Building building = findBuilding(Id);
        _buildings.removee(building);
        --_buildingNumber;
    }
